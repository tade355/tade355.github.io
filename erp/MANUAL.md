...
- **Tentative Cost** — a five-line field estimate, all using standard flat rates rather than logged ledger entries: **Rental Cost** (Partnership/Rented dozers on this roster, days worked this period × their day rate), **Diesel Cost** (Fuel Used × the diesel rate in effect that day, summed across the roster), **Site Logistics** (₦12,800 per working day this period — any day at least one roster dozer had an operation), **Diesel Logistics** (₦1,500 per 30 litres of diesel used this period), and **Operator Cost** (₦30,000 per 8 hours worked, for Company and Partnership dozers only — a Rented dozer's day rate already includes the owner's own operator).
...
- **M/c Recovered / Maintenance Incurred / Net M/c Recovered** — M/c Recovered has two parts: the Management Fee retained on this project's Partnership/Rented dozers (days worked × fee/day, the same bookkeeping a Dozer Rent Payments settlement uses), plus the rental rate saved by using Company-owned dozers instead of renting equivalent capacity (days worked × the dozer's Rental Rate/Day, if one is set — no rent is actually paid out, so this is value generated/cost avoided). Either way, it's shown net of the whole roster's Maintenance Log repair costs this period (Maintenance Incurred).
...
> - **Provisional Revenue** = quantity × the contract rate in effect that day **for that report's Operation Type** (Rate History), summed across every Daily Operations report logged for the project — a same-day figure, available the moment a report is submitted, whether or not it's been invoiced yet. This is the number to check if an active project shows ₦0 Verified Revenue but real work has clearly gone in — it means the work hasn't been invoiced yet, not that nothing happened.
